...
- **full live 能力跑不起來是預期的**（非全部）：共享 gate（`fold_in_sandbox_gate.py`）、治理詞彙定義
  （`PG-xxx` / `DDR-031` / `Slop #n` / `engine-locus`）**缺**；文中對它們、以及對未發佈的 `solo-pipeline` /
  `_integration` 的散文提及，都是 dangling reference，**已知且接受**，不是 bug、不要「修」。
  但：**每個沙盒都有可一鍵跑綠的 test 入口**（見 README「快速驗證」），且 `.claude/commands/` 已隨附**清理版
  `/command` 入口**（`self-correcting-loop` · `sandbox-openshell`；turbovec DOC-ONLY 無）。各沙盒的真實 live
  運作與生產消費關係見各 `RUN.md` + 頂層 `PRODUCTION-CONSUMPTION.md`。
...
.claude/commands/    ← 清理版 /command 入口（self-correcting-loop.md · sandbox-openshell.md）
sandboxes/
...
- ❌ 不要為了「讓它能跑」而去重建私有 gate / 治理層——那超出本鏡像範圍（`/command` 入口已附清理版，不需重建）。
