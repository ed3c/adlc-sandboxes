...
> `sandboxes/self-correcting-loop/src/loop_kernel.py`（純 python3，本 repo 即真實可跑）。實際運作過程與
> 真實輸出見同沙盒的 `RUN.md`。
...
FINAL/ITERATING，使「FINAL」蘊含於真實分數而非 agent 自評（確定性，零 LLM-judge）。
...
- **邊界**：本 command 只**驅動迭代 + 注入狀態 + 裁 DECIDE**。
  **target + rubric 由人提供；FINAL 結果由人接受**。kernel 回 `no_progress` / `exhausted` =
  **SURFACE 交人**（report-only），**永不**自動接受 / 自動無限續跑。
