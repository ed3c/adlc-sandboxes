...
- 這些是**原始 DR 綜述**（raw Gemini DR export），**不是經查證的事實**。吸收 northstar 時對它們做過 external-verify
...
- 那份逐條 external-verify 的**裁決帳本（method-problem-bridge）刻意未公開**（內含 northstar 內部架構）。
  請把這兩份 DR 當**原始吸收輸入**讀，**不是 vetted fact**。
- turbovec DR 的公式/數字以 **base64-PNG 內嵌**（Gemini DR 匯出特性），在 GitHub 上會 render 成圖。
- 來源：私有 `local_stack` 的 `docs/research/`，原檔名保留。
